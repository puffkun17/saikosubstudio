...
5. **空态 = Stage desk**：奶油工作面整面即落点（休息态几乎无边；拖入才出柑橘虚线）。结构固定为：**一句主句** → **一颗柑橘主 CTA「选择字幕」** → **静音文字链「文件夹」**；隐私 caption 挂在主句下（或角落戳记二选一，勿两处重复）；格式仅作**右下角脚注**（tiny mono）。禁止：卡中卡、双 `--hero` 并排、标题上方格式扇列、居中格式条、营销 intro、四宫格亮点轨。
