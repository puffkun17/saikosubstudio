...
5. **空态 = 本机添加**：语义只谈本机文件/文件夹，禁止暗示云端、URL、库引用。动词统一为「添加」（拖放与点选同义）。结构：**主句「添加本地字幕」** → **「拖到此处，或点选下方」+「仅在本机读取」** → **横排等权双入口「选择文件 / 选择文件夹」**（同尺寸同权重，禁止一主一次；用 `.ui-action--secondary` 横排按钮，禁止高白方砖卡片）。整面奶油虚线舞台为拖放落点。格式仅右下角脚注。禁止：营销 intro、四宫格亮点轨、格式英雄位、单侧偏袒的整面点击。
