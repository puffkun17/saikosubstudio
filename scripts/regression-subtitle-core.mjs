...
// Fetch known-good pre-corruption blob, inject coverage extras, then run.
const BASE_URL =
...
async function main() {
  ...
  const extra = readFileSync(join(dir, 'regression-coverage-extra.inc.js'), 'utf8');
  ...
  code = code.slice(0, brace) + extra + code.slice(brace);
