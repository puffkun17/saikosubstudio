...
- CSP 允许 `script-src 'wasm-unsafe-eval' 'unsafe-eval'` 与 `worker-src 'self' blob:`，供本机 RAR/7z（libarchive WASM worker）读取；缺少该项时压缩包预览会永久卡住。worker 使用版本化路径并禁止长缓存，避免浏览器沿用旧 CSP 响应头。
