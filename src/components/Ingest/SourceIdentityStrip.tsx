 ...
 * Keeps the main checklist card free for long filenames / tracks.
 */
export const SourceIdentityStrip: React.FC = () => {
  ...
  return (
    <aside className="source-identity-rail v4-panel flex w-full flex-col overflow-hidden rounded-lg">
            ...
            >
              {isSearchingTmdb ? (
                ...
                <>
                  ...
                  <span className="px-3 text-center text-[11px] leading-relaxed text-[var(--v4-text-faint)]">
                    封面按 2:3 预留
                  </span>
                </>
              )}
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-3 p-3.5 md:p-4">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h3
              className="min-w-0 text-[15px] font-semibold leading-snug tracking-tight text-[var(--v4-text)]"
              title={displayTitle}
            >
              {displayTitle}
            </h3>
          ...
          {displayOriginal ? (
            <p
              className="mt-1 line-clamp-2 text-[12px] font-medium leading-snug text-[var(--v4-text-muted)]"
            ...
            !tmdbData && (
              <p className="mt-1 text-[12px] leading-snug text-[var(--v4-text-muted)]">
                用于命名与预览背景，可稍后补充。
              </p>
            )
          )}
          ...
          {overview ? (
            <p className="mt-2.5 line-clamp-5 text-[12px] leading-relaxed text-[var(--v4-text-muted)]" title={overview}>
              {overview}
            </p>
          ) : null}
        </div>
        ...
        <AnimatePresence initial={false}>
          {showInlineNotice && filmNotice && (
            ...
            >
              <div className="rounded-lg border border-[color:rgba(197,164,114,0.28)] bg-[color:rgba(28,22,14,0.72)] px-2.5 py-2">
                <div className="flex items-start gap-2">
                  <p className="min-w-0 flex-1 text-[12px] leading-relaxed text-[#f0e6d4]">
